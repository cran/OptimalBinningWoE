...
private:
  ...
  void initial_binning() {
    ...
    for (size_t i = 0; i < feature_.size(); ++i) {
      ...
      if (it == bin_map.end()) {
        ...
        bin.categories.push_back(cat); bin.count++; bin.count_pos += val; bin.count_neg += (1 - val);
      } else {
        auto& bin = it->second;
        bin.count++;
        bin.count_pos += val;
  ...
  void merge_bins_in_temp(std::vector<CategoricalBin>& temp, size_t idx1, size_t idx2) {
